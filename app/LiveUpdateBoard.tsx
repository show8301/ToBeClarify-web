...
function LiveMarqueeRow({people,reverse=false,onNavigate}:{people:StaffSummary[];reverse?:boolean;onNavigate:(event:React.MouseEvent<HTMLAnchorElement>)=>void}){
  ...
  return <div ref={rowRef} className={`live-marquee-row${reverse?" is-reverse":""}`} aria-label={reverse?"今晚待命店員第二排":"今晚待命店員第一排"} onPointerDown={pause} onPointerUp={resume} onPointerCancel={resume} onWheel={()=>{pausedUntil.current=Date.now()+350}} onMouseEnter={()=>{focused.current=true}} onMouseLeave={()=>{focused.current=false;resume()}} onFocus={()=>{focused.current=true}} onBlur={()=>{focused.current=false;resume()}}>
