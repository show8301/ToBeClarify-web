...
const navigation = [
  { label: "營運操作", items: [["00", "點單管理"], ["01", "訂單列表"], ["02", "營運總覽"], ["03", "店員管理"]] },
  { label: "內容與設定", items: [["04", "菜單設定"], ["05", "活動設定"], ["06", "首頁設定"]] },
];
...
export default function AdminOrderListPreviewPage() {
  return <div className="adminTheme"><main className="adminShell adminMockShell">
    <aside className="adminTopbar">
      <div className="adminTopbarBrand"><span className="adminTopbarMark"><img src="/favicon.ico" alt="" /></span><span className="adminTopbarBrandCopy"><strong>清醒夢</strong><small>LUCID DREAM</small></span><button className="adminBackToSite adminBrandSiteLink" type="button">↗ 公開網站</button></div>
    ...
    <div className="adminWorkspace"><div className="adminContent"><section className="adminPage adminOrderListPage">
      ...
      <div className="adminOrderListLayout"><section className="adminOrderListTableWrap"><table className="adminOrderListTable"><thead><tr><th>訂單編號</th><th>顧客</th><th>送出時間</th><th>狀態</th><th>金額</th><th aria-label="開啟訂單明細" /></tr></thead><tbody>{rows.map((row, index) => <tr className={index === 0 ? "isSelected" : ""} key={row.id}><td><strong>{row.number}</strong></td><td><strong>{row.customer}</strong><small>ID {row.gameId}</small></td><td>{row.time}</td><td><span className={`adminOrderStatus is-${row.tone}`}>{row.status}</span></td><td><strong>{row.total}</strong></td><td><button type="button" aria-label={`查看訂單 ${row.number}`}><span aria-hidden="true">›</span></button></td></tr>)}</tbody></table></section>
