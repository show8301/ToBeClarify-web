...
const statusLabels = { submitted: '等待確認', partially_confirmed: '部分確認', needs_reschedule: '需重新排程', confirmed: '已成立', in_service: '服務中', completed: '已完成', cancelled: '已取消', expired: '已失效', rejected: '已退回' };
...
export default function OrderClient() {
  ...
  const cartCount = cart.meals.reduce((sum, line) => sum + line.quantity, 0)
    + cart.nominations.filter((line) => !line.serviceRemoved || !line.baseRemoved).length + cart.tips.length;
  ...
  const submit = async () => {
    const incomplete = cart.nominations.some((line) => line.serviceRemoved && !line.baseRemoved);
    if (incomplete) {
    ...
    setLoading(true);
    try {
      await orderingApi.submit(token, {
        meals: cart.meals.map(({ referenceId, kind, quantity }) => ({ referenceId, kind, quantity })),
        nominations: cart.nominations.filter((line) => !line.serviceRemoved).map((line) => ({
          staffId: line.staffId, serviceId: line.serviceId, segmentCount: line.segments,
      ...
      setSession((await orderingApi.access(token)).session);
      setNotice({ message: '訂單已送出。指名服務將在被指名店員確認後成立。', error: false });
      setTab('orders');
    } catch (error) {
      setNotice({ message: error.message, error: true });
    ...
    <main className="orderShell">
      ...
      <section className="orderWorkspace">
        <div className="orderMain">
          ...
          {tab === 'orders' ? <MyOrders orders={orders} /> : null}
...
function NominationPage({ session, settings, staff, cart, setCart, onNotice }) {
  ...
  useEffect(() => { setServiceId(''); setSegments(1); }, [staffId]);
  ...
  const already = cart.nominations.filter((line) => !line.serviceRemoved).length;
  const add = () => {
    if (!selectedStaff || !service) return onNotice({ message: '請先選擇店員與服務項目。', error: true });
    if (already >= session.maxNominatedStaff) return onNotice({ message: `本次最多可同時指名 ${session.maxNominatedStaff} 位店員。`, error: true });
    if (cart.nominations.some((line) => line.staffId === selectedStaff.id && !line.serviceRemoved)) return onNotice({ message: '同一張訂單每位店員只能加入一次；追加時數請另開新訂單。', error: true });
    setCart((current) => ({ ...current, nominations: [...current.nominations, { id: crypto.randomUUID(), staffId: selectedStaff.id, staffName: selectedStaff.displayName, serviceId: service.id, serviceName: service.serviceName, segments: safeSegments, participants, startsAt, duration: service.durationMinutes || safeSegments * settings.segmentMinutes, baseFee: settings.baseNominationFee, baseTotal, serviceUnit, serviceTotal, serviceRemoved: false, baseRemoved: false }] }));
    onNotice({ message: '指名服務與基礎指名費已分列加入本次點餐。', error: false });
  };
  return <div className="orderPage"><PageHeading kicker="STAFF FIRST" title="指名服務" text={`先選店員，再查看該店員提供的服務。每節 ${settings.segmentMinutes} 分鐘，最多同時指名 ${session.maxNominatedStaff} 人。`} />
    ...
    {selectedStaff ? <section className="nominationSection"><header><span>02</span><div><h2>{selectedStaff.displayName} 的服務</h2><p>有明定時間的服務按單次收費；未明定時間則每節都計一次服務費。</p></div></header>
      <div className="nominationServiceGrid">{services.map((item) => <button key={item.id} className={serviceId === item.id ? 'isActive' : ''} onClick={() => { setServiceId(item.id); setSegments(item.durationMinutes ? Math.ceil(item.durationMinutes / settings.segmentMinutes) : 1); }}><span>{item.serviceType === 'special' ? 'SPECIAL' : 'SERVICE'}</span><strong>{item.serviceName}</strong><p>{item.serviceDescription}</p><footer><b>{money(item.price)}</b><small>{item.durationMinutes ? `${item.durationMinutes} 分鐘／單次` : '每節計費'}</small></footer></button>)}</div>
    </section> : null}
    {service ? <section className="nominationComposer"><div><span>03 / 排程與節數</span><h2>{selectedStaff.displayName}｜{service.serviceName}</h2></div><div className="nominationControls">
      ...
      <label>參與人數<div className="stepper"><button type="button" onClick={() => setParticipants(Math.max(1, participants - 1))}>−</button><strong>{participants}</strong><button type="button" onClick={() => setParticipants(Math.min(20, participants + 1))}>＋</button></div></label>
    </div><div className="nominationPrice"><div><span>基礎指名費</span><b>{money(settings.baseNominationFee)} × {safeSegments} 節</b><strong>{money(baseTotal)}</strong></div><div><span>服務費</span><b>{money(serviceUnit)} {service.durationMinutes ? '× 1 次' : `× ${safeSegments} 節`}</b><strong>{money(serviceTotal)}</strong></div><footer><span>本項合計</span><strong>{money(baseTotal + serviceTotal)}</strong></footer></div><button className="orderPrimaryAction" type="button" onClick={add}>加入本次點餐</button></section> : null}
  </div>;
}
...
function CartPage({ cart, setCart, session, subtotal, onSubmit, loading }) {
  ...
  const removeNominationPart = (id, part) => setCart((current) => ({ ...current, nominations: current.nominations.map((line) => line.id !== id ? line : { ...line, [part === 'service' ? 'serviceRemoved' : 'baseRemoved']: true }).filter((line) => !(line.serviceRemoved && line.baseRemoved)) }));
  ...
  return <div className="orderPage"><PageHeading kicker="REVIEW" title="查看明細" text="快速刪除每個選項；指名服務必須先刪除服務項目，才能刪除相依的基礎指名費。" />
    {empty ? <EmptyState title="本次點餐還是空的" text="從一般點餐、指名服務或小費頁加入項目。" /> : <div className="cartLayout"><div className="cartLines">
      ...
      {cart.nominations.map((item) => <div className="cartNominationGroup" key={item.id}><header><span>{item.staffName}</span><small>{item.segments} 節 · {item.duration} 分鐘 · {new Date(item.startsAt).toLocaleString('zh-TW')}</small></header>{!item.serviceRemoved ? <CartLine label="服務項目" title={item.serviceName} price={item.serviceTotal} onRemove={() => removeNominationPart(item.id, 'service')} /> : <div className="cartRemoved">服務項目已刪除，現在可刪除基礎指名費。</div>}{!item.baseRemoved ? <CartLine label="基礎指名費" title={`${money(item.baseFee)} × ${item.segments} 節`} price={item.baseTotal} disabled={!item.serviceRemoved} removeHint={!item.serviceRemoved ? '請先刪除服務項目' : ''} onRemove={() => removeNominationPart(item.id, 'base')} /> : null}</div>)}
    ...
    </div><aside className="cartSummary"><h2>本次結算</h2><dl><div><dt>品項小計</dt><dd>{money(subtotal)}</dd></div><div className="isCredit"><dt>信物折抵（僅餐點）</dt><dd>− {money(credit)}</dd></div><div className="isTotal"><dt>本次應付</dt><dd>{money(subtotal - credit)}</dd></div></dl><p>指名訂單送出後仍須等待被指名店員確認。追加服務時數請另開新訂單。</p><button disabled={loading} type="button" onClick={onSubmit}>確認並送出訂單</button></aside></div>}
  </div>;
}
...
function MyOrders({ orders }) {
  ...
  return <div className="orderPage"><PageHeading kicker="ORDER HISTORY" title="我的訂單" text="查看今天每次加點的狀態、排程、金額與狀態歷程。" />
    {orders.length === 0 ? <EmptyState title="今天還沒有訂單" text="完成本次點餐後，訂單會出現在這裡。" /> : <div className="myOrderList">{orders.map((order) => <article key={order.id} className={`myOrderCard status-${order.status}`}><button className="myOrderHead" type="button" onClick={() => setOpen(open === order.id ? '' : order.id)}><div><span>{order.orderNumber}</span><strong>{statusLabels[order.status] || order.status}</strong></div><div><small>{new Date(order.submittedAt).toLocaleString('zh-TW')}</small><b>{money(order.totalAmount)}</b></div></button>{open === order.id ? <div className="myOrderDetail"><div className="myOrderStage"><span>{order.queueStage}</span>{order.queueMinutes ? <small>已等待 {order.queueMinutes} 分鐘</small> : null}</div><div className="myOrderItems">{order.items.map((item) => <div key={item.id}><span>{item.name}</span><b>{money(item.lineTotal)}</b></div>)}</div>{order.nominees.map((item) => <div className="myOrderNominee" key={item.id}><strong>{item.staffName}｜{item.serviceName}</strong><span>{item.segmentCount} 節 · {new Date(item.requestedStartsAt).toLocaleString('zh-TW')}</span><small>店員狀態：{item.confirmationStatus}</small></div>)}<dl><div><dt>小計</dt><dd>{money(order.subtotal)}</dd></div><div><dt>信物折抵</dt><dd>− {money(order.mealCreditApplied)}</dd></div><div><dt>應付</dt><dd>{money(order.totalAmount)}</dd></div></dl><ol className="orderTimeline">{order.history.map((item, index) => <li key={`${item.createdAt}-${index}`}><span>{new Date(item.createdAt).toLocaleTimeString('zh-TW', { hour: '2-digit', minute: '2-digit' })}</span><div><strong>{statusLabels[item.toStatus] || item.toStatus}</strong><p>{item.reason}</p></div></li>)}</ol></div> : null}</article>)}</div>}
  </div>;
}
