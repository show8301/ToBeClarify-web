...
當 Web 異動只涉及畫面呈現、版面、樣式或互動呈現，且不改動 API、資料模型、API 契約、權限或伺服器行為時，先在本機完成畫面迭代，再進入測試環境發布：

1. 先啟動本地開發伺服器，提供受影響頁面的本地網址供檢視。
2. 使用者檢視本地畫面期間，持續在本地累積多次微調，完成後再整理成同一批變更。
3. 本地畫面確認後，提交並推送 Web 變更至 `dev`，依測試環境政策執行允許的 build、靜態／設定、部署狀態與 HTTP 可用性檢查；除非使用者明確要求，不執行自動化測試套件。
4. 不為純畫面修改部署 API；完成測試環境確認後，仍依下方正式發布流程等待使用者確認，再由 `dev` → `main` 手動推進。

若異動包含 API、資料、權限或伺服器行為，依該範圍適用的一般 Web／API 開發與發布流程處理。
