...
export function AdminNotificationsPage(){
  ...
  return <AdminPage title="通知中心" eyebrow="Notifications" description="設定點餐與香檳塔提醒；關閉提示不代表接單或取消訂單。"><div className="notification-center"><div className="adminTabs"><button disabled={busy} onClick={()=>changeScope(false)}>我的通知</button>{['manager','developer'].includes(user?.role||'')&&<button disabled={busy} onClick={()=>changeScope(true)}>店內廣播</button>}</div>
