...
import {AdminButton,AdminField,AdminPage,AdminPanel} from '@/features/admin/shared/AdminShared.jsx';
import {notificationRequest,notificationSoundUrl} from '@/features/admin/notifications/api';

import {Bell, BellRing} from 'lucide-react';
import {NotificationDeviceCard, NotificationWorkspace} from '@/features/admin/notifications/NotificationWorkspace';
import {NotificationSoundLibrary} from '@/features/admin/notifications/NotificationSoundLibrary';
...
export function AdminNotificationsPage(){
  ...
  const [caps,setCaps]=useState<NotificationCapabilities|null>(null);const [reload,setReload]=useState(0);
  useEffect(()=>{
  ...
  const changeScope=(value:boolean)=>{if(value===broadcast)return;if(dirty&&!window.confirm('目前有尚未儲存的通知草稿，切換頁籤會捨棄草稿。要繼續嗎？'))return;setBusy(true);setError('');setMessage('');setDirty(false);setSettings({revision:0,schemaVersion:2,rules:[]});setCaps(null);setBroadcast(value);};
  const update=(index:number,key:keyof NotificationRule,value:NotificationRule[keyof NotificationRule])=>{if(busy)return;setMessage('');setDirty(true);setSettings(current=>({...current,rules:current.rules.map((x,i)=>i===index?{...x,[key]:value}:x)}));};
  const mutateRules=(mutator:(rules:NotificationRule[])=>NotificationRule[])=>{if(busy)return;setMessage('');setDirty(true);setSettings(current=>({...current,rules:mutator(current.rules)}));};
  ...
  const reloadSettings=()=>{if(dirty&&!window.confirm('目前有尚未儲存的通知草稿，重新載入會捨棄草稿。要繼續嗎？'))return;setBusy(true);setError('');setMessage('');setReload(value=>value+1);};
  return <div className="notification-page"><AdminPage title="通知中心" eyebrow="Notifications" description="設定點餐、指名、營業時段與各種情境提醒；關閉彈窗不代表接單或取消訂單。"><div className="notification-center">
    <NotificationDeviceCard center={center}/>
    <div className="notification-scope-tabs" role="group" aria-label="通知設定範圍"><button type="button" aria-pressed={!broadcast} disabled={busy} onClick={()=>changeScope(false)}>我的通知</button>{['manager','developer'].includes(user?.role||'')&&<button type="button" aria-pressed={broadcast} disabled={busy} onClick={()=>changeScope(true)}>店內廣播</button>}</div>
    {error&&<div className="adminNotice" role="alert"><p>{error}</p><AdminButton variant="ghost" disabled={busy} onClick={reloadSettings}>重新載入</AdminButton></div>}{message&&<p className="adminNotice" role="status">{message}</p>}
    <NotificationWorkspace key={broadcast?'broadcast':'personal'} settings={settings} broadcast={broadcast} busy={busy||!caps} dirty={dirty} labels={labels} availableTypes={availableTypes} staff={staff} makeRule={makeRule} onRulesChange={mutateRules} onSave={save} onDiscard={reloadSettings}
      renderFields={(rule,index)=><div className="notification-rule-grid">
