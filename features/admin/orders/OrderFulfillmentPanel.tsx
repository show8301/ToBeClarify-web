...
const labels: Record<string, string> = {
  ...
  cancel: "取消未服務項目", reschedule: "改期", carry_forward: "轉入新營業期／保留", waiting: "待接單", accepted: "已接單", in_service: "處理中",
...
function UnitControls({ orderId, unit, onChanged }: { orderId: string; unit: Unit; onChanged: () => Promise<void> }) {
  ...
  return <div className="adminOrderEmergency">
    ...
    {unit.allowedActions.length > 0 && <>
      ...
      <div className="adminPageActions">{unit.allowedActions.filter(action => action !== "start_now").map(action => <button type="button" key={action} disabled={busy || !!retry || ((action === "cancel" || action === "reschedule" || action === "carry_forward") && !reason.trim()) || (action === "reschedule" && unit.kind === "nominee" && !startsAt) || (action === "carry_forward" && !!targetBusinessPeriodId && unit.kind === "nominee" && !startsAt)} onClick={() => void run(action)}>{labels[action]}</button>)}{unit.allowedActions.includes("start_now") && !preview && <button type="button" disabled={busy || !!retry} onClick={() => void run("start_now")}>{labels.start_now}</button>}</div>
    </>}
    {error && <p role="alert">{error}</p>}
