...
type Overview = {
  ...
  attendanceBackfillRequests: any[];
};
...
export function AdminSettlementPage() {
  ...
  return <AdminPage eyebrow="PAYROLL · SETTLEMENT" title="帳目／薪資結算" description="依營業日期鎖定規則版本，核對實收營業額、工時、小費與各角色薪資。" actions={<><AdminButton variant="secondary" onClick={() => void load()} disabled={loading || saving}>重新整理</AdminButton>{canManage && (isFinalized ? <AdminButton onClick={() => void reopen()} disabled={saving}>重新開放新時段</AdminButton> : <AdminButton onClick={finalize} disabled={saving || !overview || overview.anomalies.length > 0}>正式結算</AdminButton>)}</>}> 
    <div className="adminSettlementToolbar"><AdminField label="營業日期"><input type="date" value={date} onChange={(event) => setDate(event.target.value)} /></AdminField><AdminField label="營業時段"><input type="number" min="1" value={sessionNo} onChange={(event) => setSessionNo(Number(event.target.value) || 1)} /></AdminField><AdminField label="日期類型"><select value={dayType} onChange={(event) => setDayType(event.target.value)} disabled={!canManage || Boolean(overview?.run.status !== "draft")}><option value="normal">非活動日</option><option value="event">活動日</option></select></AdminField>{canManage ? <><AdminButton variant="secondary" onClick={saveInputs} disabled={saving || isFinalized}>保存輸入</AdminButton><AdminButton onClick={calculate} disabled={saving || isFinalized}>重新計算</AdminButton></> : null}</div>
    ...
    {!loading && !error && overview ? <>
      ...
      <AdminPanel title="補打卡申請與核准" description="活動日出席必須有打卡；補打卡需填寫理由並由開發人員或店經理核准。">
        ...
        <AdminButton variant="secondary" onClick={() => void submitBackfill()} disabled={saving || isFinalized}>提出補打卡</AdminButton>
