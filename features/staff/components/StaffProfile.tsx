...
export default function StaffProfile({ staff, index, navigation, source=null, embedded=false }:{ staff:StaffDetail; index:number; navigation:ProfileNavigation; source?:ProfileSource; embedded?:boolean }) {
  ...
  const staffSwitchTimer = useRef<number|null>(null);
  ...
  return <main className="profile-page">
    ...
    <motion.article key={currentStaff.id} className={`profile-spread${embedded?" is-order-embedded":""}`} initial={reduceMotion||isProfileTransition?false:{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{duration:.5,ease:[.22,1,.36,1]}}>
      ...
      <motion.div className="portrait-zone" onPointerDown={embedded?undefined:startSwipe} onPointerUp={embedded?undefined:finishSwipe} onPointerCancel={embedded?undefined:()=>{swipeStart.current=null}}>
        ...
        {!embedded?<span className="portrait-swipe-hint">SWIPE TO SWITCH STAFF ↔</span>:null}
      </motion.div>
    ...
    <AnimatePresence>{lightboxIndex!==null&&images[lightboxIndex]&&<motion.div className="lightbox" role="dialog" aria-modal="true" aria-label={`${currentStaff.displayName} 照片瀏覽器`} initial={reduceMotion?false:{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} transition={{duration:.28}} onPointerDown={(event)=>{if(event.target===event.currentTarget)closeLightbox()}}>
