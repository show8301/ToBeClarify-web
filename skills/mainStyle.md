...
圖片輪播、切換分頁等轉換呈現內容操作時，都要有適當的動畫


### 導覽列 navbar

目的：要將首頁、店員珍藏（店員介紹）、慶典情報（活動資訊）、艾歐澤亞週報（活動花絮）、留聲機（留言板）、店舖動態顯示於navbar
...
- 前台不一定要顯示真實金額，可用點數或次數替代。
- 若涉及薪資或內部營收，前台 API 不可暴露敏感欄位。
...
- 預設不要顯示完整帳號、Discord ID 或任何可追蹤個資。
- 後台應可設定客人是否公開上榜。
