...
export default function GuestbookWall({initial}:{initial:GuestbookPage}){
  ...
  const reload=async()=>{
    const data=await clientApiRequest<GuestbookPage>(clientApiPath("/guestbook/comments",{page:1}));
  ...
  const submit=async(event:React.FormEvent,commentId?:string)=>{
    ...
    try{
      await clientApiRequest<unknown>(commentId?`/guestbook/comments/${encodeURIComponent(commentId)}/replies`:"/guestbook/comments",{method:"POST",body:JSON.stringify({displayName,content:message,userToken})});
  ...
  const loadMore=async()=>{
    setBusy(true);
    try{
      const data=await clientApiRequest<GuestbookPage>(clientApiPath("/guestbook/comments",{page:page+1}));
    ...
    }catch(error){setStatus(error instanceof Error?error.message:"暫時無法載入更多留言。")}
    finally{setBusy(false)}
  };
