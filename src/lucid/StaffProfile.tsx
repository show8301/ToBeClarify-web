...
import { NAVIGATION_EVENT, useRouter } from "./router";
...
async function loadStaffDetail(id:string) {
  ...
  const request=(async()=>{
    ...
    try{
      const data=await clientApiRequest<StaffDetail>(`/staff-members/${encodeURIComponent(id)}`,{signal:controller.signal});
